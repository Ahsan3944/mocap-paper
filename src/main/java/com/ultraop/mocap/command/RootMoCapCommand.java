...
public final class RootMoCapCommand implements CommandExecutor, TabCompleter {
    ...
    public RootMoCapCommand(MoCapCommand base, SceneManager sceneManager) {
        ...
        this.settings = new SettingsCommand(sceneManager.getPlugin());
    }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args[0].equalsIgnoreCase("settings")) {
            if (args.length == 2) return filter(SettingsCommand.names(), args[1]);
            return List.of();
        }
    ...
    private List<String> sceneTab(String[] args) {
        ...
        else if (args.length == 3 && List.of("copy", "rename", "remove", "add_to", "remove_from", "modify", "info", "list").contains(args[1].toLowerCase(Locale.ROOT))) values.add("<scene_name>");
        else if (args.length == 4 && List.of("copy", "rename", "add_to", "remove_from", "modify").contains(args[1].toLowerCase(Locale.ROOT))) values.add(args[1].equalsIgnoreCase("add_to") ? "<recording_or_scene>" : args[1].equalsIgnoreCase("modify") ? "<element_pos>" : "<name>");
        return filter(values, args[args.length - 1]);
    }
    private List<String> filter(List<String> values, String input) { String lower=input.toLowerCase(Locale.ROOT); return values.stream().filter(v->v.startsWith(lower)||v.startsWith("<")).toList(); }
}
