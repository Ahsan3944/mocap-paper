...
public final class SettingsCommand {
    private static final List<String> NAMES = List.of(
            "playback_speed", "recording_synchronization", "block_actions_playback", "block_initialization",
            "track_entities", "play_entities", "prevent_tracking_played_entities", "on_death", "on_change_dimension",
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        plugin.getConfig().set("settings." + key, parse(key, value)); plugin.saveConfig();
    ...
    private static boolean valid(String key, String value) {
        return switch (key) {
            case "playback_speed", "hit_range" -> number(value);
            case "block_actions_playback", "block_initialization", "recording_synchronization", "prevent_tracking_played_entities", "assign_dimension", "start_instantly", "chat_recording", "chat_playback", "invulnerable_playback", "use_authlib_services" -> bool(value);
            case "on_death" -> List.of("end_recording", "split_recording", "continue_synced").contains(value.toLowerCase(Locale.ROOT));
            case "on_change_dimension" -> List.of("nothing", "end_recording", "split_recording").contains(value.toLowerCase(Locale.ROOT));
            default -> !value.isBlank();
        };
    }
    private static boolean bool(String v) { return v.equalsIgnoreCase("true") || v.equalsIgnoreCase("false"); }
    ...
    private static Object parse(String key,String value) { if(key.equals("playback_speed")||key.equals("hit_range"))return Double.parseDouble(value); if(boolSetting(key))return Boolean.parseBoolean(value); return value; }
    ...
    private static Object defaultValue(String key) { return switch(key) {
        case "playback_speed" -> 1.0; case "hit_range" -> 0.0; case "recording_synchronization","block_actions_playback","block_initialization","prevent_tracking_played_entities","assign_dimension","start_instantly","chat_recording","chat_playback","invulnerable_playback","use_authlib_services" -> true;
        case "track_entities" -> "@vehicles;@projectiles;@items"; case "play_entities" -> "*"; case "on_death","on_change_dimension" -> "end_recording"; case "dimension_source" -> "recording"; case "assign_profile","player_name_handling" -> "ignore_and_replace_casing"; default -> "";
    }; }
