...
public final class SettingsCommand {
    ...
    private static boolean valid(String key,String value){return switch(key){case "playback_speed","hit_range","entity_tracking_distance"->number(value);case "block_actions_playback","block_initialization","recording_synchronization","prevent_tracking_played_entities","assign_dimension","start_instantly","chat_recording","chat_playback","invulnerable_playback","use_authlib_services","start_as_recorded"->bool(value);case "on_death"->List.of("end_recording","split_recording","continue_synced").contains(value.toLowerCase(Locale.ROOT));case "on_change_dimension"->List.of("nothing","end_recording","split_recording").contains(value.toLowerCase(Locale.ROOT));case "dimension_source"->List.of("assigned","current","assigned_or_current").contains(value.toLowerCase(Locale.ROOT));case "player_name_handling"->List.of("ignore_casing","ignore_and_replace_casing","strict").contains(value.toLowerCase(Locale.ROOT));case "assign_profile"->List.of("no","only_name","full").contains(value.toLowerCase(Locale.ROOT));default->!value.isBlank();};}
    ...
    private static Object parse(String key,String value){if(key.equals("playback_speed")||key.equals("hit_range")||key.equals("entity_tracking_distance"))return Double.parseDouble(value);if(boolSetting(key))return Boolean.parseBoolean(value);return value.toLowerCase(Locale.ROOT);}
    private static boolean boolSetting(String k){return List.of("block_actions_playback","block_initialization","recording_synchronization","prevent_tracking_played_entities","assign_dimension","start_instantly","chat_recording","chat_playback","invulnerable_playback","use_authlib_services","start_as_recorded").contains(k);}
    private static Object defaultValue(String key){return switch(key){case "playback_speed"->1.0;case "hit_range","entity_tracking_distance"->key.equals("entity_tracking_distance")?128.0:0.0;case "recording_synchronization","block_actions_playback","block_initialization","prevent_tracking_played_entities","assign_dimension","chat_playback","invulnerable_playback"->true;case "start_instantly","chat_recording","use_authlib_services","start_as_recorded"->false;case "track_entities"->"@vehicles;@projectiles;@items";case "play_entities"->"*";case "on_death","on_change_dimension"->"end_recording";case "dimension_source"->"assigned_or_current";case "assign_profile"->"no";case "player_name_handling"->"ignore_casing";default->"";};}
