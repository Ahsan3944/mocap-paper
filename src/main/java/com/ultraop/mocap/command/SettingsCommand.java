...
public final class SettingsCommand {
    ...
    private static boolean valid(String key,String value){return switch(key){case "playback_speed","hit_range","entity_tracking_distance"->number(value);case "block_actions_playback","block_initialization","recording_synchronization","prevent_tracking_played_entities","assign_dimension","start_instantly","chat_recording","chat_playback","invulnerable_playback","use_authlib_services"->bool(value);case "on_death"->List.of("end_recording","split_recording","continue_synced").contains(value.toLowerCase(Locale.ROOT));case "on_change_dimension"->List.of("nothing","end_recording","split_recording").contains(value.toLowerCase(Locale.ROOT));case "dimension_source"->List.of("assigned","current","assigned_or_current").contains(value.toLowerCase(Locale.ROOT));case "player_name_handling"->List.of("ignore_casing","ignore_and_replace_casing","strict").contains(value.toLowerCase(Locale.ROOT));case "assign_profile"->!value.isBlank();default->!value.isBlank();};}
