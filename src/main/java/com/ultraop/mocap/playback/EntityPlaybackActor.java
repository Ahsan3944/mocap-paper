...
public final class EntityPlaybackActor {
    ...
    private void applyNbt(String nbt){try{CompoundTag tag=TagParser.parseTag(nbt);if(entity.getWorld() instanceof org.bukkit.World world){var level=((CraftWorld)world).getHandle();((org.bukkit.craftbukkit.entity.CraftEntity)entity).getHandle().load(TagValueInput.create(ProblemReporter.DISCARDING,level.registryAccess(),tag));}}catch(Exception ignored){}}
