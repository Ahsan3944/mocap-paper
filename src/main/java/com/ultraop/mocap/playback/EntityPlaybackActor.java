...
public final class EntityPlaybackActor {
    ...
    private final Entity entity; private final double scale; private final boolean invulnerablePlayback; private final boolean originalPersistent;
    ...
    public EntityPlaybackActor(Entity entity,double scale){this(entity,scale,true);} public EntityPlaybackActor(Entity entity,double scale,boolean invulnerablePlayback){this.entity=entity;this.scale=scale;this.invulnerablePlayback=invulnerablePlayback;this.originalPersistent=entity.isPersistent();String nbt=consumePlayerAsEntityNbt();if(nbt!=null)applyNbt(nbt);entity.addScoreboardTag(MOCAP_ENTITY_TAG);applySavePolicy();applyScale();}
