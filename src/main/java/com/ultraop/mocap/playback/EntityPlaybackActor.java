...
public final class EntityPlaybackActor {
    public static final String MOCAP_ENTITY_TAG="mocap_entity"; private static final ThreadLocal<String> QUEUED_PLAYER_AS_ENTITY_NBT=new ThreadLocal<>();
    ...
    public static void queuePlayerAsEntityNbt(String nbt){if(nbt==null||nbt.isBlank())QUEUED_PLAYER_AS_ENTITY_NBT.remove();else QUEUED_PLAYER_AS_ENTITY_NBT.set(nbt);} private static String consumePlayerAsEntityNbt(){String nbt=QUEUED_PLAYER_AS_ENTITY_NBT.get();QUEUED_PLAYER_AS_ENTITY_NBT.remove();return nbt;}
    ...
    private void applySavePolicy(){if(JavaPlugin.getProvidingPlugin(EntityPlaybackActor.class).getConfig().getBoolean("settings.prevent_saving_entities",true))entity.setPersistent(false);} private void applyNbt(String nbt){try{CompoundTag tag=TagParser.parseCompoundFully(nbt);var level=((CraftWorld)entity.getWorld()).getHandle();((CraftEntity)entity).getHandle().load(TagValueInput.create(ProblemReporter.DISCARDING,level.registryAccess(),tag));}catch(Exception ignored){}}
    ...
    public void remove(){if(!entity.isValid())return;switch(mode()){case REMOVE->entity.remove();case KILL->{if(entity instanceof LivingEntity living){living.setInvulnerable(false);living.setHealth(0.0);}else entity.remove();}case LEFT_UNTOUCHED->{restoreState();}case RELEASE_AS_NORMAL->{restoreState();entity.removeScoreboardTag(MOCAP_ENTITY_TAG);if(entity instanceof Mob mob)mob.setAI(true);}}}
    private void restoreState(){entity.setPersistent(originalPersistent);entity.setGravity(originalGravity);if(entity instanceof LivingEntity living){living.setInvulnerable(originalInvulnerable);if(living.getAttribute(Attribute.SCALE)!=null)living.getAttribute(Attribute.SCALE).setBaseValue(originalScale);}}
    private static AfterPlayback mode(){String value=JavaPlugin.getProvidingPlugin(EntityPlaybackActor.class).getConfig().getString("settings.entities_after_playback","remove");try{return AfterPlayback.valueOf(value.toUpperCase(java.util.Locale.ROOT));}catch(IllegalArgumentException ignored){return AfterPlayback.REMOVE;}} private void applyScale(){if(!(entity instanceof LivingEntity living)||!Double.isFinite(scale)||scale<=0.0)return;if(living.getAttribute(Attribute.SCALE)!=null)living.getAttribute(Attribute.SCALE).setBaseValue(scale);} private static ItemStack clone(ItemStack item){return item==null?null:item.clone();}
}
