...
public final class PlaybackManager {
    public enum DimensionSource { ASSIGNED, CURRENT, ASSIGNED_OR_CURRENT }
    ...
    private void loadSettings(){playbackSpeed=Math.max(0.0,plugin.getConfig().getDouble("settings.playback_speed",1.0));blockActionsPlayback=plugin.getConfig().getBoolean("settings.block_actions_playback",true);blockInitialization=plugin.getConfig().getBoolean("settings.block_initialization",true);invulnerablePlayback=plugin.getConfig().getBoolean("settings.invulnerable_playback",true);playEntities=new EntityFilter(plugin.getConfig().getString("settings.play_entities",EntityFilter.ALL.expression()));preventTrackingPlayedEntities=plugin.getConfig().getBoolean("settings.prevent_tracking_played_entities",true);try{dimensionSource=DimensionSource.valueOf(plugin.getConfig().getString("settings.dimension_source",DimensionSource.ASSIGNED_OR_CURRENT.name()).toUpperCase(Locale.ROOT));}catch(IllegalArgumentException ignored){dimensionSource=DimensionSource.ASSIGNED_OR_CURRENT;}}
    public boolean isBlockActionsPlayback(){return blockActionsPlayback;} public boolean isBlockInitialization(){return blockInitialization;} public boolean isInvulnerablePlayback(){return invulnerablePlayback;} public boolean isPlayEntities(){return playEntities.enabled();} public EntityFilter getPlayEntities(){return playEntities;} public boolean isPreventTrackingPlayedEntities(){return preventTrackingPlayedEntities;} public DimensionSource getDimensionSource(){return dimensionSource;}
    ...
    public PlaybackSession create(RecordingSession r,Player v,PlaybackModifiers m,PositionTransformer t){return create(r,v,m,t,false);} public PlaybackSession create(RecordingSession r,Player v,PlaybackModifiers m,PositionTransformer t,boolean root){World target=resolvePlaybackWorld(r,v);PlaybackSession s=new PlaybackSession(r,v,m,t,root,blockActionsPlayback,blockInitialization,playEntities,invulnerablePlayback,preventTrackingPlayedEntities,target);return s.isStopped()?null:s;} public PlaybackSession createSubscene(RecordingSession r,Player v,PlaybackModifiers m,PositionTransformer t){return create(r,v,m,t,false);}
    ...
    private World resolvePlaybackWorld(RecordingSession r,Player viewer){if(dimensionSource==DimensionSource.CURRENT)return viewer.getWorld();String key=r.getAssignedDimensionKey();if(key!=null){World assigned=findWorld(key);if(assigned!=null)return assigned;}return viewer.getWorld();}
