...
public final class PlaybackManager {
    ...
    private SceneManager sceneManager; private BukkitTask ticker; private double playbackSpeed=1.0; private boolean blockActionsPlayback=true,blockInitialization=true,invulnerablePlayback=true,preventTrackingPlayedEntities=true; private EntityFilter playEntities=EntityFilter.ALL;
    ...
    private void loadSettings(){playbackSpeed=Math.max(0.0,plugin.getConfig().getDouble("settings.playback_speed",1.0));blockActionsPlayback=plugin.getConfig().getBoolean("settings.block_actions_playback",true);blockInitialization=plugin.getConfig().getBoolean("settings.block_initialization",true);invulnerablePlayback=plugin.getConfig().getBoolean("settings.invulnerable_playback",true);playEntities=new EntityFilter(plugin.getConfig().getString("settings.play_entities",EntityFilter.ALL.expression()));preventTrackingPlayedEntities=plugin.getConfig().getBoolean("settings.prevent_tracking_played_entities",true);}
