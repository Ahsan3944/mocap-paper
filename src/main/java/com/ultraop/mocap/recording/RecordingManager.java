...
public final class RecordingManager {
    ...
    public RecordingSession stopRecordingForPlayer(Player p){for(RecordingSession s:new ArrayList<>(active.values()))if(s.getSourcePlayerId().equals(p.getUniqueId()))return stopRecording(s.getId());return null;} public RecordingSession discard(UUID id){RecordingSession s=active.remove(id);return s!=null?s:completed.remove(id);} public RecordingSession get(UUID id){RecordingSession s=active.get(id);return s!=null?s:completed.get(id);}
    ...
    public void handleRespawn(Player oldPlayer,Player newPlayer){RecordingSession s=waitingForRespawn.remove(oldPlayer.getUniqueId());if(s==null)return;if(!s.onRespawn(newPlayer))return;active.remove(s.getId());completed.put(s.getId(),s);RecordingSession next=startRecording(newPlayer);plugin.getLogger().fine("Recording split on respawn: "+s.getId()+" -> "+next.getId());}
    private void tick(){for(RecordingSession s:new ArrayList<>(active.values())){Player p=Bukkit.getPlayer(s.getSourcePlayerId());if(p==null||!p.isOnline())continue;if(s.isDead()){waitingForRespawn.putIfAbsent(p.getUniqueId(),s);}boolean split=s.capture(p);if(split){active.remove(s.getId());completed.put(s.getId(),s);RecordingSession next=startRecording(p);plugin.getLogger().fine("Recording split on dimension change: "+s.getId()+" -> "+next.getId());}}}
