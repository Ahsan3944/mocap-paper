...
public final class RecordingManager {
    ...
    public RecordingSession stopRecording(UUID id){RecordingSession s=active.remove(id);if(s==null)return null;s.stop();complete(s);return s;}
    ...
    public RecordingSession stopRecordingForPlayer(Player p){for(RecordingSession s:new ArrayList<>(active.values()))if(s.getSourcePlayerId().equals(p.getUniqueId()))return stopRecording(s.getId());return null;} public RecordingSession discard(UUID id){RecordingSession s=active.remove(id);waitingForRespawn.remove(id);return s!=null?s:completed.remove(id);} public RecordingSession get(UUID id){RecordingSession s=active.get(id);return s!=null?s:completed.get(id);}
