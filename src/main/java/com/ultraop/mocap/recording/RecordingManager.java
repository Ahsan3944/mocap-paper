...
public final class RecordingManager {
    ...
    private double entityTrackingDistance=128.0; private EntityFilter trackEntities=EntityFilter.DEFAULT_TRACK_ENTITIES; private boolean preventTrackingPlayedEntities=true;
    ...
    public double getEntityTrackingDistance(){return entityTrackingDistance;} public EntityFilter getTrackEntities(){return trackEntities;} public boolean isPreventTrackingPlayedEntities(){return preventTrackingPlayedEntities;}
    public RecordingSession startRecording(Player p){RecordingSession s=new RecordingSession(p,entityTrackingDistance,trackEntities,preventTrackingPlayedEntities);s.setOnDeath(onDeath);s.setOnChangeDimension(onChangeDimension);active.put(s.getId(),s);return s;} public RecordingSession startRecording(Player p,String name){RecordingSession s=startRecording(p);s.setInstantSaveName(name);return s;}
