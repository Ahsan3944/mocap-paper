...
public final class RecordingSession {
    ...
    private void trackEntities(Player player){double maxDistanceSquared=ENTITY_TRACKING_DISTANCE*ENTITY_TRACKING_DISTANCE;Set<UUID> seenThisTick=new HashSet<>();for(Entity entity:player.getWorld().getEntities()){
        ...
        UUID id=entity.getUniqueId();seenThisTick.add(id);boolean wasHurt=hurt.remove(id);entityFrames.computeIfAbsent(id,ignored->new ArrayList<>()).add(EntityStateFrame.capture(entity,nextTick,wasHurt));trackedLastSeenTick.put(id,Math.toIntExact(nextTick));}
