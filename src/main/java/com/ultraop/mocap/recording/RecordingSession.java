...
public final class RecordingSession {
    ...
    private final double entityTrackingDistance;
    private final EntityFilter trackEntities;
    ...
    public RecordingSession(Player p){this(p,128.0,EntityFilter.DEFAULT_TRACK_ENTITIES);}
    public RecordingSession(Player p,double trackingDistance,EntityFilter filter){this(UUID.randomUUID(),p.getUniqueId(),p.getName(),Instant.now(),null,new ArrayList<>(),new LinkedHashMap<>(),new ArrayList<>(),Math.max(0.0,trackingDistance),filter==null?EntityFilter.disabled():filter);lastWorldKey=worldKey(p);}
    private RecordingSession(UUID id,UUID pid,String pn,Instant start,Instant stop,List<PlayerStateFrame> frames,Map<UUID,List<EntityStateFrame>> entities,List<BlockActionFrame> blocks,double trackingDistance,EntityFilter filter){this.id=id;sourcePlayerId=pid;sourcePlayerName=pn;startedAt=start;stoppedAt=stop;this.frames=new ArrayList<>(frames);entityFrames=new LinkedHashMap<>();entities.forEach((u,v)->entityFrames.put(u,new ArrayList<>(v)));blockActions=new ArrayList<>(blocks);trackedLastSeenTick=new LinkedHashMap<>();entityFrames.forEach((u,v)->{if(!v.isEmpty())trackedLastSeenTick.put(u,Math.toIntExact(v.get(v.size()-1).tick()));});nextTick=frames.stream().mapToLong(PlayerStateFrame::tick).max().orElse(-1L)+1L;lastWorldKey=frames.isEmpty()?null:frames.get(frames.size()-1).worldKey();entityTrackingDistance=trackingDistance;trackEntities=filter;}
    static RecordingSession loaded(UUID id,UUID pid,String pn,Instant start,Instant stop,List<PlayerStateFrame> f){return new RecordingSession(id,pid,pn,start,stop,f,Map.of(),List.of(),128.0,EntityFilter.DEFAULT_TRACK_ENTITIES);}
    static RecordingSession loaded(UUID id,UUID pid,String pn,Instant start,Instant stop,List<PlayerStateFrame> f,Map<UUID,List<EntityStateFrame>> e){return new RecordingSession(id,pid,pn,start,stop,f,e,List.of(),128.0,EntityFilter.DEFAULT_TRACK_ENTITIES);}
    static RecordingSession loaded(UUID id,UUID pid,String pn,Instant start,Instant stop,List<PlayerStateFrame> f,Map<UUID,List<EntityStateFrame>> e,List<BlockActionFrame> b){return new RecordingSession(id,pid,pn,start,stop,f,e,b,128.0,EntityFilter.DEFAULT_TRACK_ENTITIES);}
    ...
    boolean onRespawn(Player newPlayer){if(onDeath!=OnDeath.SPLIT_RECORDING){died=false;diedTick=-1;lastWorldKey=worldKey(newPlayer);return false;}stop();return true;}
    public boolean isDead(){return died;} public void recordBlockAction(BlockActionFrame a){if(a!=null)blockActions.add(a);} public long currentTick(){return Math.max(0,nextTick-1);}
    private void trackEntities(Player p){double max=entityTrackingDistance*entityTrackingDistance;Set<UUID> seen=new HashSet<>();for(Entity e:p.getWorld().getEntities()){if(e instanceof Player||!e.isValid()||e.getUniqueId().equals(sourcePlayerId))continue;if(p.getLocation().distanceSquared(e.getLocation())>max||isPlaybackEntity(e)||!trackEntities.matches(e))continue;UUID id=e.getUniqueId();seen.add(id);boolean wasHurt=hurt.remove(id);entityFrames.computeIfAbsent(id,x->new ArrayList<>()).add(EntityStateFrame.capture(e,nextTick,wasHurt));trackedLastSeenTick.put(id,Math.toIntExact(nextTick));}trackedLastSeenTick.keySet().removeIf(u->!seen.contains(u));}
