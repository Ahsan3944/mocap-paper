...
public final class RecordingSession {
    ...
    public RecordingSession(Player p){this(UUID.randomUUID(),p.getUniqueId(),p.getName(),Instant.now(),null,new ArrayList<>(),new LinkedHashMap<>(),new ArrayList<>());}
    ...
    void capture(Player p){if(died){captureDeadTick(p);return;}UUID u=p.getUniqueId();boolean main=swingMainHand.remove(u),off=swingOffHand.remove(u),wasHurt=hurt.remove(u);frames.add(PlayerStateFrame.capture(p,nextTick,main,off,wasHurt));trackEntities(p);if(p.isDead()){died=true;diedTick=nextTick;}nextTick++;}
