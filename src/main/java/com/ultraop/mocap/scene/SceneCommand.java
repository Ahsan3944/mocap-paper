...
public final class SceneCommand {
    ...
    private void modify(CommandSender s,String[] a)throws IOException{
        ...
        SceneElement old=scene.elements().get(p-1);
        if(a[4].equalsIgnoreCase("subscene_name")){
            requireArgs(a,5,"subscene_name <new_name>");
            List<SceneElement> elements=new ArrayList<>(scene.elements());
            elements.set(p-1,new SceneElement(a[5],old.modifiers()));
            scene.clear(); elements.forEach(scene::add); sceneManager.save(a[2],scene);
            s.sendMessage(ChatColor.GREEN+"Scene subscene renamed: "+a[5]); return;
        }
        PlaybackModifiers m=modifyModifier(old.modifiers(),a,4);List<SceneElement> elements=new ArrayList<>(scene.elements());elements.set(p-1,new SceneElement(old.name(),m));scene.clear();elements.forEach(scene::add);sceneManager.save(a[2],scene);s.sendMessage(ChatColor.GREEN+"Scene element modified.");
    }
    ...
    private PlaybackModifiers modifyTransform(PlaybackModifiers m,String[] a,int i){requireArgs(a,i+1,"transformation");String k=a[i].toLowerCase(Locale.ROOT);return switch(k){case "rotation"->m.withRotation(Double.parseDouble(requireAt(a,i+1,"degrees")));case "mirror"->m.withMirror(PlaybackModifiers.Mirror.valueOf(requireAt(a,i+1,"mirror").toUpperCase(Locale.ROOT)));case "scale"->{requireArgs(a,i+2,"scale <of_player|of_scene> <scale>");double v=nonNegative(a[i+2]);String target=a[i+1].toLowerCase(Locale.ROOT);if(target.equals("of_player"))yield m.withPlayerScale(v);if(target.equals("of_scene"))yield m.withSceneScale(v);throw new IllegalArgumentException("Scale target must be of_player or of_scene.");}case "offset"->{requireArgs(a,i+3,"offset <x> <y> <z>");yield m.withOffset(Double.parseDouble(a[i+1]),Double.parseDouble(a[i+2]),Double.parseDouble(a[i+3]));}case "config"->modifyConfig(m,a,i+1);default->throw new IllegalArgumentException("Unknown transformation modifier: "+k);};}
    private PlaybackModifiers modifyConfig(PlaybackModifiers m,String[] a,int i){requireArgs(a,i+1,"config <round_block_pos|recording_center|scene_center|center_offset> ...");var c=m.transformationConfig();String k=a[i].toLowerCase(Locale.ROOT);return switch(k){case "round_block_pos"->m.withTransformationConfig(c.withRoundBlockPos(bool(requireAt(a,i+1,"value"))));case "recording_center"->m.withTransformationConfig(c.withRecordingCenter(PlaybackModifiers.RecordingCenter.valueOf(requireAt(a,i+1,"center").toUpperCase(Locale.ROOT))));case "scene_center"->{String v=requireAt(a,i+1,"scene center");var t=PlaybackModifiers.SceneCenterType.valueOf(v.toUpperCase(Locale.ROOT));yield m.withTransformationConfig(c.withSceneCenter(t,t==PlaybackModifiers.SceneCenterType.COMMON_SPECIFIC?requireAt(a,i+2,"scene element"):null));}case "center_offset"->{requireArgs(a,i+3,"center_offset <x> <y> <z>");yield m.withTransformationConfig(c.withCenterOffset(Double.parseDouble(a[i+1]),Double.parseDouble(a[i+2]),Double.parseDouble(a[i+3])));}default->throw new IllegalArgumentException("Unknown transformation config: "+k);};}
